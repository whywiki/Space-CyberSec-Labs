...
LOG_FILE = os.path.join(REPORT_DIR, 'command_token_authentication.log')
...
class TokenCommandHandler(socketserver.StreamRequestHandler):
    def handle(self):
        users = load_user_db()
        for raw in self.rfile:
            ...
            db = users[user]
            if role != db['role'] or token != db['token']:
                msg = f'[REJECTED] AUTHENTICATION FAILED: {user}'
                print(msg); log(f'{msg} RAW={line}'); print(); continue
...
if __name__ == '__main__':
    ...
    print(f'=== TOKEN COMMAND RECEIVER STARTED ===\nListening on 127.0.0.1:{PORT}\n')
    with ReusableTCPServer(('127.0.0.1', PORT), TokenCommandHandler) as server:
